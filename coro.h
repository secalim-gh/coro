...
/*
 ...
 32   sp  (r13)
 36   lr  (r14)  <-- optional: can be used to store "return" or function pointer
 if desired
*/
...
typedef struct __attribute__((aligned(16))) Context {
  ...
  // Use a struct to ensure alignment if possible, or force alignment on the
  // whole Context struct.
...
CORODEF Coro coro_create(void *co) {
...
#if defined(__aarch64__) || defined(__arm64__)
  coro->stack = CORO_MMAP(NULL, STACK_SIZE + PAGE_SIZE, PROT_NONE,
  ...
  coro->stack =
      CORO_MMAP(NULL, STACK_SIZE + PAGE_SIZE, PROT_NONE,
                MAP_ANONYMOUS | MAP_PRIVATE | MAP_GROWSDOWN | MAP_STACK, -1, 0);
  coro->ctx.sp = (size_t)((char *)coro->stack + STACK_SIZE);
  coro->ctx.lr = (size_t)_turnaround;
  coro->ctx.fp = 0;
...
#endif
  mprotect((char *)coro->stack + getpagesize(), STACK_SIZE,
